/** * GET /api/admin/audit-logs * * Audit log query interface (Req 3.9, 21.14). * * Access rules: *  - Super Admin: can query audit logs across ALL tenants *  - Local Admin: can only query audit logs for their own tenant * * Supports filtering by: *  - tenant_id (Super Admin only) *  - user_id *  - action (exact match or prefix, e.g. 'auth.' matches all auth events) *  - entity_type *  - date range (start_date, end_date) *  - category (auth | authz | data | config | security) * * Supports pagination via limit + offset. * * Requirements: 3.9, 21.14 */import { NextRequest, NextResponse } from 'next/server';
...
export async function OPTIONS(request: NextRequest) { return handleOptionsRequest(request);}// GET /api/admin/audit-logs — query audit logs with filtering and pagination
