/** * GET  /api/admin/feature-flags  — list feature flags for a tenant (Req 23.1, 23.2) * POST /api/admin/feature-flags  — create a feature flag for a tenant (Req 23.1, 23.2) * * Requirements: 23.1, 23.2 * * Access: *  - Super Admin: can manage flags for any tenant (pass ?tenant_id=) *  - Local Admin: can only manage flags for their own tenant */import { NextRequest, NextResponse } from 'next/server';
...
const createFlagSchema = z.object({ feature_key: z.string()  .min(1, 'Feature key is required')  .max(100)  .regex(/^[a-z_]+$/, 'Feature key must be lowercase letters and underscores only'),  enabled: z.boolean().default(false),  configuration: z.record(z.unknown()).optional().nullable(),});

export async function OPTIONS(request: NextRequest) { return handleOptionsRequest(request);}// GET /api/admin/feature-flags — list all feature flags for a tenant (Req 23.1)
...
export const GET = withErrorHandler(async (request: NextRequest) => { const ctxResult = requireTenantContext(request);
  ...
  if (!allowedRoles.includes(role) && !isSuperAdmin) { return forbiddenResponse('Only administrators can manage feature flags');
}
...
const descriptions: Record<string, string> = {
    ...
    [FeatureKey.SOCIAL_PROGRAM_SHARING]: 'Social program sharing and enrollment links',
  };
...
export const POST = withErrorHandler(async (request: NextRequest) => { const ctxResult = requireTenantContext(request);
  ...
  const allowedRoles = ['local_admin'];
  if (!allowedRoles.includes(role) && !isSuperAdmin) { return forbiddenResponse('Only administrators can create feature flags');
}

const { searchParams } = new URL(request.url);

  const targetTenantId = isSuperAdmin  ? (searchParams.get('tenant_id') || tenantId)  : tenantId;
