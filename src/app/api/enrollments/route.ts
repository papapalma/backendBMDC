 ...
 *
 * On successful enrollment creation, an 'enrollment-added' event is emitted
 * to the enrollment event bus for real-time synchronization.
 *
 * Requirement Initialization (Task 6.1):
 * When an enrollment is created, enrollment requirements are automatically initialized
 * by evaluating applicability rules from requirement_definitions for the trainee.
 */
...
import { initializeEnrollmentRequirements } from '@/services/enrollmentRequirementService';
...
const { data: program, error: programError } = await supabaseAdmin
  ...
  // Check enrollment capacity (Req 13.1, 13.2, 13.3, 13.5)
  // If enrollment_limit is null, skip capacity check (unlimited)
  if (program.enrollment_limit !== null) {
    const currentEnrollment = await (await import('@/services/programService')).programService.getCurrentEnrollmentCount(
    ...
    if (currentEnrollment >= program.enrollment_limit) {
      return errorResponse(
        'Program is at capacity and cannot accept new enrollments.',
        400
      );
    }
  }
...
const enrollmentRecord = {
  ...
  // Task 6.1: Initialize enrollment requirements for this enrollment
  // This evaluates applicability rules from requirement_definitions and creates
  // enrollment_requirements records with appropriate is_applicable flags
  try {
    const enrollmentRequirements = await initializeEnrollmentRequirements(
      enrollment.id,
      validatedData.trainee_id,
      tenantId
    );

    console.log(
      `[Enrollment ${enrollment.id}] Initialized ${enrollmentRequirements.length} requirements`
    );
  } catch (requirementError) {
    // Log the error but don't fail enrollment creation
    // Requirements initialization is a secondary operation
    console.error(
      `Failed to initialize enrollment requirements for enrollment ${enrollment.id}: ${requirementError instanceof Error ? requirementError.message : String(requirementError)}`
    );
  }
  ...
  // Emit 'enrollment-added' event to event bus for real-time synchronization (Req 3.6, 3.7)
