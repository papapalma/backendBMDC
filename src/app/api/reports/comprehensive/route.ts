...
export const POST = withErrorHandler(async (request: NextRequest) => {
  ...
  // Query enrollments separately to get program and enrollment_date data
  let enrollmentsQuery = supabaseAdmin
    .from('enrollments')
    .select('id, trainee_id, program_id, status, enrollment_date, tenant_id');

  // Tenant isolation for enrollments
  if (user.role !== 'super_admin') {
    enrollmentsQuery = enrollmentsQuery.eq('tenant_id', user.tenantId);
  }
...
if (startDate) activityQuery = activityQuery.gte('created_at', startDate);
  ...
  // Build program enrollment stats from enrollments table (not trainees)
  const traineeByProgram = enrollmentRows.reduce((acc, enrollment) => {
    const pid = (enrollment.program_id as unknown) as string;
  ...
  for (const enrollment of enrollmentRows) {
    const dateKey = (enrollment.enrollment_date || '').split('T')[0];
    if (!dateKey) continue;
