...
export const POST = withErrorHandler(async (request: NextRequest) => {
  // requireTenantContext extracts tenantId from the session cookie so we can
  // inject it into every program_sessions row (NOT NULL constraint).
...
const ctxResult = requireTenantContext(request);
  ...
  if (!allowedRoles.includes(role)) {
    return forbiddenResponse('Insufficient permissions to create sessions');
  }
...
if (body.sessions && Array.isArray(body.sessions)) {
    ...
    const sessions = await sessionService.bulkCreateSessions(
...
const validatedData = createSessionSchema.parse(body);
  const session = await sessionService.createSession(validatedData, tenantId);
  await activityLogService.logAction(
    userId,
    'create',
    'session',
