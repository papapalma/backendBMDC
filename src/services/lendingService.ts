...
import { borrowingSlipService } from './borrowingSlipService';
...
await itemService.updateItemQuantity(lendingData.item_id, lendingData.quantity, 'borrow');
    ...
    // Create borrowing slip automatically when lending is created
    if (tenantId) {
      try {
        const borrowerName = lendingData.borrower_name || (lending.trainee?.name) || 'Unknown Borrower';
        const itemName = itemDetails?.name || 'Unknown Item';
        
        await borrowingSlipService.createBorrowingSlip(
          {
            lending_id: lending.id,
            item_id: lendingData.item_id,
            borrower_name: borrowerName,
            item_name: itemName,
            due_date: lendingData.expected_return_date,
            quantity: lendingData.quantity,
            item_description: itemDetails?.description,
            borrower_contact: lendingData.borrower_contact,
            notes: lendingData.notes,
          },
          userId,
          tenantId
        ...
        // Log the error but don't fail the lending creation
        console.error('Error creating borrowing slip:', slipError);
      }
    }
...
const { data: updatedLending, error } = await supabaseAdmin
    ...
    // Mark corresponding borrowing slip as returned
    try {
      const slips = await borrowingSlipService.getSlipsForLending(null, id);
      if (slips && slips.length > 0) {
        for (const slip of slips) {
          if (slip.status !== 'returned') {
            await borrowingSlipService.markAsReturned(null, slip.id, returnData.notes);
          }
        }
      }
    } catch (slipError) {
      // Log the error but don't fail the return process
      console.error('Error updating borrowing slip:', slipError);
    }
