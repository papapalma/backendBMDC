...
export interface Enrollment {
  ...
  trainee_status_record_id?: string | null; // Link to trainee_status_records table
  created_at: string;
  updated_at: string;
  // Joined relations
  trainee?: Trainee;
  program?: Program;
  traineeStatusRecord?: TraineeStatusRecord;
}
...
/**
 * TraineeStatusRecord: Post-graduation outcome tracking for trainees
 * 
 * Captures employment status, job match, and qualitative feedback after training completion.
 * One record per enrollment, tracking what happened to the trainee after graduating.
 */
export interface TraineeStatusRecord {
  ...
  graduation_date?: string | null; // DATE format (YYYY-MM-DD)
  ...
  job_start_date?: string | null; // DATE format (YYYY-MM-DD)
  job_sector?: string | null; // e.g., "Manufacturing", "Healthcare", "IT"
  
  // Skills match assessment
  // Indicates whether the job aligns with training received
  skills_match?: 'exact_match' | 'partial_match' | 'no_match' | 'not_applicable' | null;
  skills_match_percentage?: number | null; // 0-100
  
  // Qualitative feedback
  remarks?: string | null; // Admin notes on trainee outcome and job fit
  
  // Unemployment reason
  unemployment_reason?: string | null; // e.g., "No available jobs", "Personal reasons"
...
/**
 * TraineeStatusRecordInput: Data for creating/updating trainee status records
 */
export interface TraineeStatusRecordInput {
  trainee_id: string;
  enrollment_id: string;
  graduation_status: 'pending' | 'graduated' | 'not_completed' | 'suspended';
...
/**
 * TraineeStatusSummary: High-level summary of trainee outcomes
 * Used for dashboard/analytics views
 */
export interface TraineeStatusSummary {
...
/**
 * TraineeStatusFilter: Filtering options for trainee status queries
 */
export interface TraineeStatusFilter {
  graduation_status?: 'pending' | 'graduated' | 'not_completed' | 'suspended';
  employment_status?: 'pending' | 'employed' | 'unemployed' | 'self_employed' | 'pursuing_education' | 'deceased';
  ...
  date_from?: string;
  date_to?: string;
  search?: string; // search by job title, employer name, remarks
}
