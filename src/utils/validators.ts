...
const baseProgramSchema = z.object({
  ...
  type_of_funding: z.string()
    .min(1, 'Type of funding is required')
    .max(255, 'Type of funding must not exceed 255 characters')
    .trim(),
  status: z.enum(['active', 'completed', 'upcoming', 'cancelled']).optional().nullable(),
  enrollment_limit: z.number()
    .int('Enrollment limit must be a whole number')
    .min(1, 'Enrollment limit must be at least 1')
    .max(10000, 'Enrollment limit must not exceed 10,000'),
});
...
// ============================================================================
// 2FA & EMAIL VERIFICATION SCHEMAS
// ============================================================================

/**
 * OTP Code validation (6 digits, no letters)
 */
...
/**
 * Verify email during signup (trainee registration)
 */
export const verifyEmailSignupSchema = z.object({
  email: z.string()
    .email('Invalid email address')
    .max(255, 'Email must not exceed 255 characters')
    .toLowerCase()
...
/**
 * Verify login 2FA (admin users)
 */
export const verifyLogin2FASchema = z.object({
  otp: z.string()
    .regex(/^\d{6}$/, 'OTP must be exactly 6 digits'),
  tempToken: z.string()
    .min(20, 'Invalid temporary token'),
});
...
/**
 * Verify password reset OTP (first OTP)
 */
...
/**
 * Complete password reset (double OTP verification + new password)
 */
...
/**
 * Updated register schema with email verification requirement
 */
export const registerWithEmailVerificationSchema = registerSchema.extend({
  // Inherits all fields from registerSchema
  // Email verification OTP can be optional during initial registration,
  // verified in separate /verify-email-signup endpoint
});

/**
 * Login schema with email verification check (already exists as loginSchema)
 * No changes needed - verification happens in middleware
 */
export const loginWithEmailVerificationSchema = loginSchema.extend({
  // Inherits email and password from loginSchema
  // Email verification is checked during login flow
});
...
export const traineeRegistrationSchema = z.object({
  ...
  // Tenant selection (required - must be selected before registration)
...
// 2FA & Email verification types
export type OTPCodeInput = z.infer<typeof otpCodeSchema>;
export type VerifyEmailSignupInput = z.infer<typeof verifyEmailSignupSchema>;
...
export type RegisterWithEmailVerificationInput = z.infer<typeof registerWithEmailVerificationSchema>;
export type LoginWithEmailVerificationInput = z.infer<typeof loginWithEmailVerificationSchema>;



// ============================================================================
// TRAINEE STATUS VALIDATORS
// ============================================================================

/**
 * Base schema for trainee status (without refinements)
 * Contains all field definitions without validation refinements
 */
const baseTraineeStatusSchema = z.object({
  trainee_id: z.string().uuid('Invalid trainee ID'),
  enrollment_id: z.string().uuid('Invalid enrollment ID'),
  
  graduation_status: z.enum(['pending', 'graduated', 'not_completed', 'suspended'], 
    { required_error: 'Graduation status is required' }),
  graduation_date: z.string().date('Invalid graduation date format (YYYY-MM-DD)').optional().nullable(),
  certificate_id: z.string().uuid('Invalid certificate ID').optional().nullable(),
  
  employment_status: z.enum(['pending', 'employed', 'unemployed', 'self_employed', 'pursuing_education', 'deceased'], 
    { required_error: 'Employment status is required' }),
  
  // Employment details (required if employed or self_employed)
  job_title: z.string().max(255, 'Job title must not exceed 255 characters').optional().nullable(),
  employer_name: z.string().max(255, 'Employer name must not exceed 255 characters').optional().nullable(),
  job_start_date: z.string().date('Invalid job start date format (YYYY-MM-DD)').optional().nullable(),
  job_sector: z.string().max(100, 'Job sector must not exceed 100 characters').optional().nullable(),
  
  // Skills match assessment
  skills_match: z.enum(['exact_match', 'partial_match', 'no_match', 'not_applicable']).optional().nullable(),
  skills_match_percentage: z.number().min(0).max(100, 'Skills match percentage must be between 0 and 100').optional().nullable(),
  
  // Qualitative feedback
  remarks: z.string().max(2000, 'Remarks must not exceed 2000 characters').optional().nullable(),
  
  // Unemployment reason (required if unemployed)
  unemployment_reason: z.string().max(255, 'Unemployment reason must not exceed 255 characters').optional().nullable(),
});

/**
 * Schema for creating trainee status records
 * Tracks post-graduation outcomes: employment, job match, remarks
 * All identity fields required, validation refinements enforced
 */
export const createTraineeStatusSchema = baseTraineeStatusSchema
  .refine(
    (data) => {
  ...
  .refine(
    (data) => {
      // If employed/self_employed, should have employer name
      if (data.employment_status === 'employed' || data.employment_status === 'self_employed') {
...
/**
 * Schema for updating trainee status records (all fields optional)
 * Identity fields (trainee_id, enrollment_id) are omitted
 * Validation refinements enforced to maintain business rules
 */
export const updateTraineeStatusSchema = baseTraineeStatusSchema
  .partial()
  .omit({
    trainee_id: true,
    enrollment_id: true,
...
/**
 * Schema for filtering trainee status records
 */
export const traineeStatusFilterSchema = z.object({
  graduation_status: z.enum(['pending', 'graduated', 'not_completed', 'suspended']).optional(),
  employment_status: z.enum(['pending', 'employed', 'unemployed', 'self_employed', 'pursuing_education', 'deceased']).optional(),
  skills_match: z.enum(['exact_match', 'partial_match', 'no_match', 'not_applicable']).optional(),
...
// Type exports
export type CreateTraineeStatusInput = z.infer<typeof createTraineeStatusSchema>;
export type UpdateTraineeStatusInput = z.infer<typeof updateTraineeStatusSchema>;
export type TraineeStatusFilterInput = z.infer<typeof traineeStatusFilterSchema>;
